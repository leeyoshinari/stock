...
class StockController(Controller):
    ...
    async def query_stock_list(self, code: str = "", name: str = "", sortField: str = 'qrr_desc', page: int = 1, pageSize: int = 20) -> Result:
        query = model.SearchStockParam()
        query.code = code if code else ""
        query.name = name if name else ""
        query.sortField = sortField if sortField else 'qrr_desc'
        query.page = page
