...
量能异常放大：若当日成交量大于前5日平均成交量的2倍，或换手率突然激增至10%以上，需警惕‘量能高潮’后的反杀，要警惕‘高位派发’，除非有极强的一字板预期，否则不买入。
资金质量异常：不仅看最后一日主力资金，必须计算最近3日的累计资金流向。若最近3日累计为负，即便最后一日大额流入，也视为诱多信号，不准买入。
...
buyPrompt = '''你是一个精通中国A股市场的短线交易员(持股时间最长20天)，非常擅长根据技术指标分析股票，下面将给你一只股票的最近多日的数据，你需要全面仔细地分析各个指标，并判断是否应该可以买入股票。
【输出要求】
请输出分析过程和最终判断结果；返回单个JSON对象，格式是：{{"code":"603128","buy":true,"reason":"分析过程和最终判断结果"}}
【字段含义说明】
天级数据字段含义：code：股票代码；day：交易日期；current_price：当日收盘价；last_price：前一日收盘价；open_price：开盘价；max_price：最高价；min_price：最低价；volume：成交量；fund：主力资金净流入（单位：万）；turnover_rate：换手率；ma_five：5日均线；ma_ten：10日均线；ma_twenty：20日均线和布林线中轨线；qrr：量比；diff：MACD的DIFF；dea：MACD的DEA；k：KDJ的K值；d：KDJ的D值；j：KDJ的J值；trix：TRIX指标值；trma：TRIX均线；boll_up：布林线上轨线；boll_low：布林线下轨线。所有数组字段按day时间顺序排列。
当天分钟级数据字段含义：time：时间，几点几分；price：当前价格；price_avg：当前分时均线价，volume：当前分钟的成交量。
这只股票的最近10日天级数据是(请注意当前时间是{})：{}
当天分钟级数据是：{}
'''

sellPrompt = '''你是一个精通中国A股市场的短线交易员，非常擅长根据技术指标分析股票，下面将给你一只股票的买入时间、持仓成本和最近多日的数据（包括当天的实时数据），你需要全面仔细地分析各个指标，判断是否应该卖出股票。
【输出要求】
...
async def queryAI(api_host: str, model: str, auth_code: str, current_time: str, buyPrice: str, buyDate: str, k_line: str, day_line: str, logger: Logger) -> dict:
    url = f"{api_host}/api/chat"
    header = {"Content-Type": "application/json", "Connection": "keep-alive", "Authorization": f"Bearer {auth_code}"}
...
async def webSearchTopicBak(api_host: str, auth_code: str, current_date: str) -> str:
    q = f'{current_date} 中国A股市场的热门题材和热门板块 市场情绪'
    prompts = f'你需要从【联网搜索资料】中找出 {current_date} 的内容，然后分析当前市场热点题材，将筛选出的热点信息，按照“事件/政策催化 -> 市场资金反应 -> 板块表现”的逻辑链进行组织，同时还要注意风险信息。请按照热点题材汇总(热点题材用,分隔)、热点题材逻辑链分析(和热点题材汇总中的题材数量要完全一样)、市场情绪、风险提示的顺序给出回答，不要输出没用的内容。【特别注意】热点题材是股票普遍涨势很好的题材，股票普遍下跌的题材不是热点题材'
    url = f"{api_host}/api/search/ai"
    header = {"Content-Type": "application/json", "Connection": "keep-alive", "Authorization": f"Bearer {auth_code}"}
    data = {"q": q, "dateRestrict": "d", "prompts": prompts}
    res = await http.post(url=url, json_data=data, headers=header)
