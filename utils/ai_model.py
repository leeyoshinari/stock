...
prompt = '''你是一个精通股票市场交易的交易员，你精通分析股票的数据来判断是否应该买入股票，下面将会给你5只股票的数据，你需要知道每个指标的含义并深入仔细严谨地分析各种指标数据，然后判断每只股票是否应该买入，你需要尽可能识别出主力诱多、假金叉、较长的上影线、超买高位钝化等这种假信号，请你直接返回一个列表的JSON，格式是{"603128": {"buy": True, "reason": ""},"601801": {"buy": False, "reason": ""}}，buy表示是否买入，reason表示判断的简单依据。\n
