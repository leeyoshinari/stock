        ...
        .then(data => {
            ...
            data.data.forEach(item => {
                ...
                if (searchType === "5") {
                    ...
                    s += `<div id="${item.id}" class="item-list" style="height:70px;"><div><a onclick="get_stock_figure('${item.code}');">${item.name}</a></div><div><a onclick="get_stock_real_figure('${item.code}');">${item.code}</a><!--img id="copy-${item.id}" src="${prefix}/static/copy.svg" alt="" /--></div><div><a onclick="show_reason('${item.code}','${item.id}',0);">${item.price}</a><img id="ai-${item.id}" src="${prefix}/static/sell.svg" alt="" onclick="query_stock_ai('${item.code}', '${item.name}');" style="width:18px;" /></div><div><a target="_blank" href="https://quote.eastmoney.com/concept/${region}${item.code}.html#chart-k-cyq">${item.create_time}</a></div>
                      ...
                      <div class="three-price" style="cursor:pointer;color:${item.sale_time === currentDay ? "red" : ""};" onclick="show_reason('${item.code}','${item.id}',1);"><span>${item.sale_time}</span><span>${item.sale_price}</span></div><div id="${item.id}-reason" style="display:none;">${item.content}</div></div>`;
                    ...
                    let win = win_m - win_a;
                    s += `<div id="${item.id}" class="item-list" style="height:42px;"><div><a onclick="get_stock_figure('${item.code}');">${item.name}</a></div><div><a onclick="get_stock_real_figure('${item.code}');">${item.code}</a></div><div><a onclick="show_reason('${item.code}','${item.id}',0);">${item.price}</a></div><div>${item.create_time.split(" ")[0]}</div>
                    ...
                    s += `<div id="${item.id}" class="item-list" style="height:42px;"><div><a onclick="get_stock_figure('${item.code}');">${item.name}</a></div><div><a onclick="get_stock_real_figure('${item.code}');">${item.code}</a><!--img id="copy-${item.id}" src="${prefix}/static/copy.svg" alt="" /--></div><div><a onclick="show_reason('${item.code}','${item.id}',0);">${item.price}</a><img id="ai-${item.id}" src="${prefix}/static/sell.svg" alt="" onclick="query_stock_ai('${item.code}', '${item.name}');" style="width:18px;" /></div><div><a target="_blank" href="https://quote.eastmoney.com/concept/${region}${item.code}.html#chart-k-cyq">${item.create_time}</a></div>
                        <div style="color:${item.sale_time === currentDay ? "red" : ""};"><a onclick="show_reason('${item.code}','${item.id}',1);">${item.sale_price}</a></div><div style="color:${item.sale_time === currentDay ? "red" : ""};">${item.sale_time}</div><div style="color:${win > 0 ? "red" : win < 0 ? "green" : ""};">${win ? (win*100).toFixed(2) + "%" : null}</div>${deleteR}<div id="${item.id}-reason" style="display:none;">${item.content}</div></div>`;
                }
            })
...
function query_stock_ai(code, name) {
    show_modal_cover();
    let site = localStorage.getItem('site');
    fetch(`${prefix}/sell/stock?site=${site}&code=${code}&price=&t=`)
        .then(res => res.json())
        .then(data => {
